...
Note: SGD requires ~10× higher learning rates than Adam and benefits from
Nesterov momentum, gradient clipping, label smoothing, and stronger
data augmentation.

Usage:
    python train_sgd.py --data-path /path/to/dataset --output-dir results_sgd
    python train_sgd.py --data-path /path/to/BUSI --output-dir results_exp2
    python train_sgd.py --data-path /path/to/BUS-UCLM --output-dir results_exp4
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train 17 CNNs with SGD vs SAM+SGD on breast US data.")
    parser.add_argument('--data-path', type=str, required=True,
                        help='Path to ImageFolder dataset.')
    parser.add_argument('--output-dir', type=str, default='results_sgd',
                        help='Directory for outputs (default: results_sgd).')
    parser.add_argument('--config', type=str,
                        default='configs/sgd_config.yaml',
                        help='YAML config file (default: configs/sgd_config.yaml).')
...
def run_experiment(model_name, use_sam, cfg, train_loader, val_loader,
    ...
    Returns:
        result_dict, history, cost_dict
    """
    opt_label = "SAM+SGD" if use_sam else "SGD"
    ...
    if use_sam:
        optimizer = SAM(head_params, base_optimizer=torch.optim.SGD,
                        lr=head_lr, momentum=momentum, nesterov=nesterov,
                        weight_decay=weight_decay, rho=sam_rho)
        base_opt = optimizer.base_optimizer
    else:
        optimizer = torch.optim.SGD(head_params, lr=head_lr,
    ...
    if use_sam:
        optimizer = SAM(param_groups, base_optimizer=torch.optim.SGD,
                        momentum=momentum, nesterov=nesterov,
                        weight_decay=weight_decay, rho=sam_rho)
        base_opt = optimizer.base_optimizer
    else:
...
def main():
    ...
    for model_name in models_to_run:
        # --- SGD baseline ---
        res_sgd, hist_sgd, cost_sgd = run_experiment(
            model_name, use_sam=False, cfg=cfg,
            train_loader=train_loader, val_loader=val_loader,
            num_classes=num_classes, class_names=class_names,
            device=device, out_dir=args.output_dir)
        all_results.append(res_sgd)
        all_costs.append(cost_sgd)
        all_histories[(model_name, 'SGD')] = hist_sgd
        ...
        all_histories[(model_name, 'SAM+SGD')] = hist_sam

        # Overlay
        plot_overlay_curves(hist_sgd, hist_sam, model_name,
                            'SGD', 'SAM+SGD',
    ...
    sgd_df = df[df['Optimizer'] == 'SGD'].set_index('Model')
    sam_df = df[df['Optimizer'] == 'SAM+SGD'].set_index('Model')

    summary = pd.DataFrame({
        ('SGD', 'Accuracy'):    sgd_df['Accuracy'],
        ('SGD', 'Precision'):   sgd_df['Precision'],
        ('SGD', 'Recall'):      sgd_df['Recall'],
        ('SGD', 'F1 Score'):    sgd_df['F1 Score'],
        ('SAM+SGD', 'Accuracy'):   sam_df['Accuracy'],
        ('SAM+SGD', 'Precision'):  sam_df['Precision'],
        ('SAM+SGD', 'Recall'):     sam_df['Recall'],
        ('SAM+SGD', 'F1 Score'):   sam_df['F1 Score'],
    })
    summary.columns = pd.MultiIndex.from_tuples(summary.columns)

    print(f"\n{'=' * 100}")
    print("  TABLE — Performance: SGD vs SAM+SGD")
    print(f"{'=' * 100}")
    ...
    # Summary plots
    # ---------------------------------------------------------------
    plot_performance_comparison(sgd_df, sam_df, models_to_run,
                                'SGD', 'SAM+SGD', args.output_dir)
    plot_improvement_heatmap(sgd_df, sam_df, 'SGD', 'SAM+SGD',
                             args.output_dir)
    ...
    wins = (sam_df['Accuracy'] > sgd_df['Accuracy']).sum()
    ties = (sam_df['Accuracy'] == sgd_df['Accuracy']).sum()
    total = len(models_to_run)
    print(f"\n  SAM+SGD wins: {wins}/{total}  |  "
          f"Ties: {ties}/{total}  |  "
          f"SGD wins: {total - wins - ties}/{total}")
